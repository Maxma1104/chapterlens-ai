...
create or replace function public.consume_analysis_quota(
  ...
  p_monthly_budget_usd numeric default 25
)
...
begin
  ...
  if v_monthly_cost >= p_monthly_budget_usd then
  ...
  on conflict (user_id, usage_date) do update
    ...
    where public.daily_usage.analysis_count < p_daily_limit
  ...
  if v_count is null then
    ...
    return query select true, greatest(0, p_daily_limit - v_count), (v_date + 1)::timestamp at time zone 'utc', 'allowed'::text;
  end if;
end;
$$;

revoke all on function public.consume_analysis_quota(integer, numeric) from public;
grant execute on function public.consume_analysis_quota(integer, numeric) to authenticated;
