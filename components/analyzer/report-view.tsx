...
export function ReportView({ report, source, onReset }: { report: AnalysisReport; source: string; onReset: () => void }) {
  ...
  const [feedback, setFeedback] = useState<"up" | "down" | "sent" | "error" | null>(null);
  ...
  return <div className="report-view">
    <div className="report-header">
      ...
      <div className="report-actions"><span className="grounded-score"><i /> {Math.round(report.overallConfidence * 100)}% {report.meta.verificationMode === "entailment" ? "support verified" : "citation match"}</span><button onClick={onReset}><RotateCcw size={15} /> New</button></div>
    </div>
    ...
    <div className="report-scroll">
      ...
      <div className="report-feedback"><span>{feedback === "sent" ? "Thanks—your feedback is recorded." : feedback === "error" ? "Feedback could not be saved. Sign in or retry shortly." : "Was this review useful?"}</span>{feedback !== "sent" && <div><button className={feedback === "up" ? "active" : ""} onClick={() => sendFeedback(true)}><ThumbsUp size={13} /> Yes</button><button className={feedback === "down" ? "active" : ""} onClick={() => sendFeedback(false)}><ThumbsDown size={13} /> Not yet</button></div>}</div>
